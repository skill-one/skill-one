...
- **何时检测** —— 启动时、窗口每次重新变为可见时，以及每小时一次的兜底（覆盖持续隐藏的会话）。
  `src/lib/update-store.ts` 把三者合并节流到**每个会话最多每 6 小时一次**请求；设置浮窗的
  「软件更新」行带 `force` 跳过节流。确认弹窗打开期间、以及正在安装期间都不会发起检测——此时
...
- **检查失败时** —— *后台*失败保持静默，且不会收回已经发现的更新，所以一次网络抖动不会把用户正
  要点击的角标抹掉；只有显式 `force` 的检查才在「软件更新」行报错。两种情况下重试窗口都随连续失败次数翻倍
  ——2 分钟、4 分钟……封顶 6 小时——因此离线或长期不可用的环境会退化为低频轮询，而不是每次唤醒
  都打一次注定失败的请求。
- **请求都是实时的** —— 检测由 Rust（`reqwest`）发出，不走 webview，绝不读 HTTP 缓存。它读的是
  GitHub 的 `releases/latest` 指针，该指针有边缘缓存：新 Release 可能要一分钟后才对客户端可见。
- **发现新版本时** —— 不再弹模态打断。左侧窄轨底部的「设置」上方会出现一枚绿色「有新版本」徽章；

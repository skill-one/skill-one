...
The snapshot also ships a Chinese page for a subset of skills at `profiles/{id}/skill_zh.md` (no index row marks it; the file's presence is the only signal). In Chinese mode the detail drawer leads the body with that page and fetches the English `SKILL.md` only when the reader flips the drawer to the original through the header's 查看原文 toggle (one toggle for the description and the body together) — or right away for an untranslated skill, whose English body is the only one there is (a disk read reads the English file alone). The Chinese page's fetch is garnish like the repos sidecar: a missing page, network failure or server error all hand the body over to the English original.

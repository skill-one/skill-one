...
The snapshot also ships a Chinese page for a subset of skills at `profiles/{id}/skill_zh.md` (no index row marks it; the file's presence is the only signal). In Chinese mode the detail drawer leads the body with that page and keeps the English `SKILL.md` one 查看原文 toggle away; an untranslated skill (or a disk read) falls back to the English body. The fetch is garnish like the repos sidecar: a missing page, network failure or server error all leave the English body in place.

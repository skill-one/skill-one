...
- `INDEX_SPEC` in [index-stream.ts](../src/lib/registry/index-stream.ts) pins `repo` / `path: "skills.jsonl"` / `ref: "dist"` (the ref above is supplied per download), and the three sidecars are addressed relative to it: `upstream/stats.json`, `upstream/trending.json`, `upstream/repos.jsonl`.
...
- `upstream/trending.json` (the trending view's top-100 ids, in upstream rank order) is fetched pinned to the same snapshot tag once it is known, so the leaderboard is read from the same snapshot as the index. An absent or unreachable list simply hides the section.

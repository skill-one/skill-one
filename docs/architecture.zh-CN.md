...
- **`src/lib/skill-detail-api.ts`**：按需拉取单个 skill 的 `SKILL.md`，解析 frontmatter 与正文；同时拉取快照可选的中文页面（`profiles/{id}/skill_zh.md`），中文模式下详情正文以它为主——此时英文 `SKILL.md` 只在读者切换「查看原文」（或该 skill 没有翻译）时才拉取。

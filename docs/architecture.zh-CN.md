...
9. 排序有一条规则和一处例外：浏览列表按工具栏的排序选择排列（默认按热度，也就是每行显示的那个「安装量 × Star 数」合成分，另一项是按名称）；只要 query 非空，worker 一律按相关度返回（名称命中 > 仓库 > 描述，热度作为同分时的 tie-break），不受该选择影响。此时工具栏把排序下拉换成只读的「相关度」标签，避免控件声称一种结果列表并没有遵守的顺序。

...
5. 搜索要等整个数据集就绪：MiniSearch 模糊索引在流结束后一次性构建（每个快照都重建的代价高于下载本身），在 worker 报告 `ready` 之前搜索框保持禁用。因此任何查询都不会基于不完整的注册表作答——索引存在之前 worker 一律返回空，作为禁用态字段的兜底。
...
9. 排序有一条规则和一处例外：浏览列表按工具栏的排序选择排列（默认按热度，也就是每行显示的那个「安装量 × Star 数」合成分，另一项是按名称）；只要 query 非空，worker 一律按相关度返回，不受该选择影响，此时工具栏把排序下拉换成只读的「相关度」标签，避免控件声称一种结果列表并没有遵守的顺序。这个顺序是：查询的每个词都必须命中（仅当没有任何一条同时命中全部词时才退化为命中任意一个词）；名称精确命中或前缀命中排在最前，它们之间按热度排——名称命中已经确定了「这是什么」，同名之间热度才是真正有意义的差别；其余按字段权重排列（名称 4 : 仓库 2 : 描述 0.5，描述只给半权，因为 SKILL.md 的描述基本都是「何时调用」的触发短语），热度只做微调。

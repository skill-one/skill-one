...
- **`src/lib/skill-detail-api.ts`**：按需拉取单个 skill 的 `SKILL.md`，解析 frontmatter 与正文；同时拉取快照可选的中文页面（`profiles/{id}/skill_zh.md`），中文模式下详情正文以它为主——此时英文 `SKILL.md` 只在读者通过头部的「查看原文」开关把整个抽屉切回原文（或该 skill 没有翻译）时才拉取。
...
| `src/components/app-header.tsx` | 应用外壳，只有一行：行首是品牌与双入口分段导航，行尾是当前列表的搜索框与设置入口——同时承担窗口拖拽区，这正是 overlay 标题栏留给应用的活。列表里的一页只会失去搜索框 |
...
| `src/components/list-facets.tsx` | 当前列表的分类 chips，与单位切换同处该列表内容区首行：能放几个放几个，其余收进「更多」浮层 |
| `src/lib/list-view.ts` | 这些控件背后的共享视图：两页共用一个查询，各列表另有自己的单位与范围 |
| `src/lib/facet-overflow.ts` | 列表首行能放几个 chip——对测量宽度的纯算术 |
...
| `src/pages/explore/search-results.tsx` | 统一的搜索答案，搜索进行时由两个可搜索列表渲染同一段实现：固定顺序的三个段落——本地已安装（磁盘上匹配的安装，行由页面注入）、应用商店（注册表命中，取数在此）、skills.sh（实时答案，对商店段去重，取数在此）。三个段落共用随单位切换的同一种形状；空段隐藏，三段全空时以「搜索中」占位等待所有请求返回后才说「未找到」。详情抽屉只走打开技能所在的那一段，并穿着那一段的表面 |
| `src/components/collapsible-section.tsx` | 所有吸顶分组头共用的外壳——「我的」页的时间分组与搜索视图的三个来源分段都走它：一行由折叠箭头、来源图标、标题和计数徽标组成，分组内容滚过时钉在顶部，点击整行即可折叠/展开。折叠后计数仍留在头上；状态为非受控，因为答案每次变化都会整体重挂载列表 |
| `src/hooks/use-installed-search.ts` | 已安装列表对共享搜索查询的回答：磁盘记录过共享名称索引，再按行联入来源账本与商店条目——与已安装列表浏览时同一条管道 |
| `src/pages/explore/live-groups.ts` | skills.sh 实时搜索结果按仓库重新归组，供搜索视图的仓库视图使用：把去重后的命中按 `owner/repo` 分桶——桶保持端点自己的相关度顺序，桶内 skill 按安装量降序。单位开关决定 live 段由什么构成：这里是仓库卡片（与所有搜索卡片一样不截断——卡片就是完整的 live 答案），技能视图则是一行一个 skill 的扁平列表。live 卡片的底栏只是标签——行（点击打开 skills.sh）是唯一的出路 |
...
10. 较长的仓库网格会按名次切成便于阅读的排名段——Top 25、26–50，此后每段边界倍增（51–100、101–200、……，见 `lib/registry/rank-bands.ts`）——每段就是一个可折叠分区，样式与「我的」列表的时间分组完全一致。读者还没滚到的段不会挂载；25 个仓库及以下的回答（按领域筛选后的常见形态）保持一张平铺网格；按领域筛选时，分段切的是该领域自身的排名。

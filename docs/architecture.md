...
| `src/components/section-header.tsx` | One section of a composite answer, as a header row: the title, a one-line aside saying where the rows come from, and the count at the far edge — the shared label of the search view's three sections |

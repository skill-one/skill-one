...
- **`src/lib/skill-detail-api.ts`**: Fetches a single skill's `SKILL.md` on demand and parses its frontmatter and body. Also fetches the snapshot's optional Chinese page (`profiles/{id}/skill_zh.md`) that leads the detail body in Chinese mode — where the English file itself is fetched only when the reader flips the drawer to the original through the header's 查看原文 toggle (or the skill ships no translation).
...
| `src/components/app-header.tsx` | The app's chrome, one row: the brand and the two-destination segmented navigation leading it, the current list's search field and the settings entry closing it, and the window's drag region — which is what the overlay title bar leaves to the app. A page inside a list loses only the search field |
...
| `src/components/app-nav.tsx` | The app's navigation: the two destinations as one segmented control in the header (active over the whole route family, including drill-down pages) |
| `src/components/list-facets.tsx` | The scope chips of the list on screen, opening that list's own first row beside its unit switch: as many chips as the line holds, and a 更多 flyout for the rest |
| `src/lib/list-view.ts` | The shared view behind those controls: one query for both lists, and each list's own unit and scope |
| `src/lib/facet-overflow.ts` | How many chips fit on the list's first row — pure arithmetic over measured widths |
...
| `src/pages/explore/search-results.tsx` | The unified search answer, rendered by both searchable lists while a search is live: three sections in a fixed order — 本地已安装 (the installs on disk that match, rows injected by the page), 应用商店 (the registry's matches, fetched here), skills.sh (the live answer, deduped against the store's, fetched here). One shape per unit across all three sections; empty sections hide, and an all-empty answer holds a searching line until every request has answered. The detail drawer walks the section the open skill came from, wearing that section's surface |
| `src/components/collapsible-section.tsx` | The shell every sticky group header is drawn through — the installed list's time buckets and the search view's three source sections alike: a header row of a disclosure chevron, the source glyph, the title and a count badge, pinning while its section passes and folding that section on a press of the row. The count stays on the header while folded, and the state is uncontrolled because every answer change remounts the list |
| `src/hooks/use-installed-search.ts` | The installed list's answer to the shared search query: the on-disk records through the shared name index, with the ledger and store-entry lookup joined in per row — the same pipeline the installed list browses with |
| `src/pages/explore/live-groups.ts` | The live skills.sh answer re-filed by repository, for the search view's repository unit: buckets the deduped hits by their `owner/repo`, keeping the endpoint's own relevance order for the buckets and each bucket's skills most-installed first. The unit switch decides what the live section is made of — repository cards here (uncapped under the search, like every search card: the card is the whole live answer), one flat row per skill in the skill unit. A live card's bar is a label — the rows, which open skills.sh, are the only way out |
...
10. A long repository grid is cut into rank bands for reading — Top 25, then 26–50, and every boundary after that doubles (51–100, 101–200, …, `lib/registry/rank-bands.ts`) — each band drawn as one collapsible section exactly like the installed list's time buckets. A band the reader has not scrolled to is not mounted yet; an answer of 25 repositories or fewer (the common shape under a domain filter) stays one flat grid, and under a domain filter the bands cut that domain's own ranking.

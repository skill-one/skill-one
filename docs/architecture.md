...
- **`src/lib/skill-detail-api.ts`**: Fetches a single skill's `SKILL.md` on demand and parses its frontmatter and body. Also fetches the snapshot's optional Chinese page (`profiles/{id}/skill_zh.md`) that leads the detail body in Chinese mode — where the English file itself is fetched only when the reader flips to it (or the skill ships no translation).

...
2. The hero carousels are the leaderboards in `featured-rankings.ts`, in display order: `trending` (skills.sh's own trending id list, walked in upstream rank) and `popular` (the whole registry ranked by install count). Both label each entry with the same install count a row displays, so a leaderboard number and a list number mean the same thing.

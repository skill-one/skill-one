...
| `src/pages/explore/live-groups.ts` | The live skills.sh answer re-filed by repository, for the store list's repository unit: buckets the deduped hits by their `owner/repo`, keeping the endpoint's own relevance order for the buckets and each bucket's skills most-installed first. The unit switch decides what the live section is made of — repository cards here, one flat row per skill in the skill unit |

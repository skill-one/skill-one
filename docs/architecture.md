...
- **`src/lib/search-index.ts`**: The store's single search entry point — a MiniSearch index over the fields each list searches, shared by the skill registry, the repos grid and the installed list. It defines what counts as a match (see *Browsing the skill list*, step 9), including the mixed-script tokenizer that keeps Chinese text searchable; `src/lib/search-skills.ts` adds the registry's own ranking on top (field boosts, popularity, name tiers). The registry builds its index inside the worker, the small per-page lists build one in a `useMemo`.
...
9. Ordering has one rule and one exception: the browsed list follows the toolbar's sort choice (popularity by default — the blended installs+stars figure each row shows, name as the alternative), while a search is always answered in relevance order whatever that choice says, and the toolbar swaps its sort dropdown for a read-only 相关度 label so it never claims an order the results do not follow. The repos grid applies the same rule — a query orders its own results and the toolbar shows 相关度 — while the ranking described below is the skill registry's own. That order is: every query term must match (the search falls back to any term only when nothing matches them all); exact and prefix name hits rank first, ordered among themselves by popularity — a name match already settles *what* the skill is, so among namesakes popularity is the difference that matters; the rest follows field weight (name 4 : repo 2 : description 0.5 — the description is half-weighted because SKILL.md descriptions are mostly trigger phrases) with popularity as a nudge.

What counts as a match has one definition, in `lib/search-index.ts`, used by all three searchable lists (the skill registry, the repos grid, and the installed skills): a query term must equal an indexed term or be the start of one, and nothing else is forgiven — a mistyped word, or a fragment from the middle of a word, matches nothing. Prefix matching is kept because a half-typed word is an unfinished query rather than a wrong one. Tokenization is mixed-script: Latin words split on whitespace and punctuation, while each Han run is additionally indexed as its single characters and overlapping bigrams, so a Chinese skill description is searchable without word boundaries. (The rows highlight a matched term wherever it occurs, so a Han match inside a longer run is marked too.)
...
2. The hero carousels are the leaderboards in `featured-rankings.ts`, in display order: `trending` (skills.sh's own trending id list, walked in upstream rank) and `popular` (the whole registry ranked by popularity). Both label each entry with the same blended installs-and-stars figure a row displays, so a leaderboard number and a list number mean the same thing.

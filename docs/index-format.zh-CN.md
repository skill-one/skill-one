...
快照还为部分 skill 附带中文页面，路径为 `profiles/{id}/skill_zh.md`（索引行没有对应标记，文件是否存在是唯一信号）。中文模式下详情抽屉的正文以该页面为主，英文 `SKILL.md` 保留在一步「查看原文」切换之后；未翻译的 skill（或磁盘读取）回退到英文正文。这次拉取与 repos 旁车一样是装饰性的：页面缺失、网络故障或服务端错误都只留下英文正文。

...
- 快照版本**直接以仓库标签（tag）解析**：列出最新的 `dist-<date>` 标签（优先 GitHub tags API，jsDelivr data API 作回退——见 [index-stream.ts](../src/lib/registry/index-stream.ts) 的 `resolveLatestTag`），因此下载地址就是上游 CI 发布的那个标签。两次列表请求都**带打散缓存的时间戳**：一个负责报告新鲜度的可变指针绝不能从缓存里拿，否则旧快照会被当成当前版本。
- `stats.json` 随后**定址到该标签读取**——地址不可变，因此不打散缓存——从中取得发布时间戳与行数。若读不到，仅凭标签仍可锚定正文下载（只是失去「未变化则跳过」的短路优化）。若完全列不出任何标签，则走旧路径：打散缓存地探测可变 `dist` 分支上的 `stats.json`，并从其 `finishedAt` 推导标签。
...
- 若标签与统计均无法解析，则退回可变的 `dist` ref——此时必须打散缓存，因为没有定址锚点时，一份一天前的边缘副本与当前索引无从分辨。
...
- 「设置」页报告当前服务的快照（`dist-<date>` 标签、发布时间、发布行数），以及本次启动是重新下载还是复用了本地缓存；在线身份到来之前会回退显示已记录的标签。其按钮会强制重下，即使快照没有变化。
